...
function validateScene(document, options = {}) {
  const limits = limitsWith(options);
  const errors = [];
  ...
  ref(
    document.scene.zOrder.length === nodes.size && document.scene.zOrder.every((id) => nodes.has(id)),
    "/scene/zOrder"
  );
...
function validateSceneOnly(document, options = {}) {
  if (!document_structural_default(document))
    return {
      ok: false,
