...
function createEditorStore(options) {
  ...
  let past = [], future = [];
  let saved = canonicalizeContent(checked.value), disposed = false;
  let gesture;
  ...
  function trim() {
    const bytes = (list) => list.reduce((sum, doc) => sum + new TextEncoder().encode(JSON.stringify(doc)).length, 0);
    while (past.length + future.length > historyLimits.maxEntries || 2 * (bytes(past) + bytes(future)) > historyLimits.maxBytes) {
