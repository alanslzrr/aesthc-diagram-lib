...
import { CARD_R, CARD_TEXT_X } from '../theme'

/** Shared authored-node metrics. No DOM, React, stylesheets or editor state. */
export function nodeGeometry(node: PlacedNode, hasVisual = false) {
  const isMuted = node.weight === 'muted',
    isEvent = node.shape === 'event'
  ...
  const hitHeight = isEvent
    ? below
      ? node.y + 30 - (node.cy - 12)
      : node.cy + 12 - (node.y - 36)
    : node.h
  return {
    centeredLabel,
    textX,
    radius:
      node.shape === 'state' || node.shape === 'terminal'
        ? Math.min(CARD_R * 2.4, node.h / 2)
        : CARD_R,
...
export function tableFieldGeometry(node: PlacedNode, index: number, key?: string) {
  return {
