import type { DiagramDocument, GraphDiagramSpec, Result } from './types'
import { nodesOf, edgesOf } from './model'
...
export function convertToGraph(
  ...
  const spec: GraphDiagramSpec = {
    ...
    nodes: nodesOf(document.spec).map((n) => {
      ...
      return {
        ...
        ...('fields' in n ? { fields: structuredClone(n.fields), shape: 'table' as const } : {}),
  ...
  if (document.spec.type !== 'graph')
    losses.push({
      path: '/spec/type',
      reason:
        'Type-specific ordering, membership and layout semantics become explicit graph geometry.',
    })
