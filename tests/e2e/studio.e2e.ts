...
test('Studio resize handle previews, cancels, commits once and supports keyboard undo', async ({
  ...
  await expect(node).not.toHaveAttribute('width', before!)
  await page.keyboard.press('Escape')
  await page.mouse.up()
  await expect(node).toHaveAttribute('width', before!)
  await node.click()
  await page.mouse.move(box.x + box.width / 2, box.y + box.height / 2)
  await page.mouse.down()
  await page.mouse.move(box.x + box.width / 2 + 70, box.y + box.height / 2 + 30, { steps: 5 })
  ...
  await handle.focus()
  await page.keyboard.press('ArrowRight')
  await expect(node).toHaveAttribute('width', String(Number(before) + 1))
  await page.keyboard.press('ControlOrMeta+z')
  await expect(node).toHaveAttribute('width', before!)
})
...
test('Studio selects connections and edits manual routes through draggable waypoints and anchors', async ({
  ...
  const waypoints = page.locator('[data-waypoint][data-waypoint-index]')
  await expect(waypoints).not.toHaveCount(0)
  const before = await page.locator('[data-waypoint-index="0"]').getAttribute('cx')
  const dot = page.locator('[data-waypoint-index="0"]')
  const box = await dot.boundingBox()
  if (!box) throw Error('missing waypoint')
  await page.mouse.move(box.x + box.width / 2, box.y + box.height / 2)
  await page.mouse.down()
  ...
  await page.mouse.up()
  await edge.click()
  expect(await page.locator('[data-waypoint-index="0"]').getAttribute('cx')).toBe(before)
  const box2 = await page.locator('[data-waypoint-index="0"]').boundingBox()
  if (!box2) throw Error('missing waypoint after reselect')
  await page.mouse.move(box2.x + box2.width / 2, box2.y + box2.height / 2)
  await page.mouse.down()
  await page.mouse.move(box2.x + box2.width / 2 + 40, box2.y + box2.height / 2 + 20, { steps: 4 })
  await page.mouse.up()
  expect(await page.locator('[data-waypoint-index="0"]').getAttribute('cx')).not.toBe(before)
  await page.getByRole('button', { name: 'Undo', exact: true }).click()
  expect(await page.locator('[data-waypoint-index="0"]').getAttribute('cx')).toBe(before)
...
test('Studio edits graph ports and ER table fields through the structured inspector', async ({
  ...
  await page.goto('/studio.html')
  await page.getByRole('button', { name: 'Order API', exact: true }).click()
  const ports = page.getByRole('region', { name: 'Ports', exact: true })
  await expect(ports.getByText('inbound', { exact: true })).toBeVisible()
  await ports.getByLabel('Port direction 1', { exact: true }).selectOption('both')
  const inbound = page.locator('[data-port="inbound"]')
  const box = await inbound.boundingBox()
  if (!box) throw Error('missing port handle')
  await page.mouse.move(box.x + box.width / 2, box.y + box.height / 2)
  await page.mouse.down()
  await page.mouse.move(box.x + box.width / 2 + 40, box.y + box.height / 2 + 40, { steps: 4 })
  await page.keyboard.press('Escape')
  await page.mouse.up()
  await page.getByRole('button', { name: 'Order API', exact: true }).click()
  await expect(ports.getByLabel('Port side 1', { exact: true })).toHaveValue('left')
  const box2 = await inbound.boundingBox()
  if (!box2) throw Error('missing port handle after cancel')
  await page.mouse.move(box2.x + box2.width / 2, box2.y + box2.height / 2)
  await page.mouse.down()
  await page.mouse.move(box2.x + box2.width / 2 + 40, box2.y + box2.height / 2 + 40, { steps: 4 })
  await page.mouse.up()
  expect(await ports.getByLabel('Port side 1', { exact: true }).inputValue()).not.toBe('left')
  await page.getByRole('button', { name: 'Add port', exact: true }).click()
  await expect(ports.getByText('inbound', { exact: true })).toHaveCount(1)
  await expect(ports.locator('li')).toHaveCount(3)
  await page.getByRole('button', { name: 'Undo', exact: true }).click()
  await expect(ports.locator('li')).toHaveCount(2)
  page.on('dialog', (d) => void d.accept())
  await page.getByRole('button', { name: 'Import JSON', exact: true }).click()
  await page.locator('input[type="file"]').setInputFiles('tests/fixtures/editor/er-document.json')
  await page.getByRole('button', { name: 'User', exact: true }).click()
  const fields = page.getByRole('region', { name: 'Table fields', exact: true })
  await expect(fields.getByLabel('Field name 1', { exact: true })).toHaveValue('id')
  await fields.getByLabel('Field name 1', { exact: true }).fill('uuid_id')
  await expect(page.getByRole('button', { name: 'User', exact: true })).toHaveCount(1)
  await fields.getByRole('button', { name: 'Add field', exact: true }).click()
  await expect(fields.locator('li')).toHaveCount(2)
  await fields.getByLabel('Field key 2', { exact: true }).selectOption('unique')
  await page.getByRole('button', { name: 'Undo', exact: true }).click()
  await expect(fields.locator('li')).toHaveCount(2)
  await page.getByRole('button', { name: 'Undo', exact: true }).click()
