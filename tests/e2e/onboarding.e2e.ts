...
test('release onboarding is explicit and generated pages link to their source', async ({
  page,
}) => {
  await page.goto('/')
  await expect(page.locator('[data-release-availability]')).toContainText('Available on npm')
  ...
  await expect(page.getByRole('main')).toContainText('is available on npm')
